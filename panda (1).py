...
def checkDfJournyPaths(data):
    for data_row_count in range(1, len(data)):
...
def checkDfJournyPath(data, data_row_index, data_row_count, path = ""):
    ...
    if path != "":
        path += " > "
    for data_row_field_index in range(1, len(data[data_row_index - 1]) - 2):
        if data[data_row_index - 1][data_row_field_index]:
            ...
            if data_row_index == data_row_count:
                ...
                if df_result_list_index == False:
                    if data[data_row_index - 1][13]:
                        dj = DfJounryResult(check_path, 1, data[data_row_index - 1][12], 0)
                    else:
                    ...
                    if data[data_row_index - 1][13]:
                        ...
                        df_result_list[df_result_list_index].total_conversion_value = df_result_list[df_result_list_index].total_conversion_value + data[data_row_index - 1][12]
                    else:
...
for df_index, df_row in df.iterrows():
    # df_data_index = checkDfJounry(str(df_row[0]))
    # if df_data_index == False:
